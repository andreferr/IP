        ...
        for (int k=0;k<jogadas;k++) {
            ...
            if (matriz[coordenadas[k][0]][coordenadas[k][1]] == -1) {
                System.out.println("Essa casa ja foi selecionada.");
                    ...
                    for (int x = coordenadas[k][0], y = coordenadas[k][1]; x < matriz.length; x++) {
                        if (matriz[y][x] == -1) {
                        } else {
                            soma[0] += matriz[y][x];
                        }
                    }
                    for (int x = coordenadas[k][0], y = coordenadas[k][1]; y < matriz.length; y++) {
                        if (matriz[y][x] == -1) {
                        } else {
                            soma[1] += matriz[y][x];
                        }
                    }
                    for (int x = coordenadas[k][0], y = coordenadas[k][1], q = 0, z = 0; (y + q) < matriz.length - 1 || (x + z) < matriz.length - 1; q++, z++) {
                        if (matriz[y][x] == -1) {
                        } else {
